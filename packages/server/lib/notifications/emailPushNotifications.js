...
async function handleEmailNotifications(params) {
  try {
...
function emailNotificationIsEnabled({
  noteType,
  toUser
}) {
  const isPersonal = noteType === 'reward' || noteType === 'reply' || noteType === 'mention';
...
async function getDefaultEmailHtml({
  urls,
  fromUser,
  post,
  toUser,
  ...
  const noteHtml = fromUser ? `<a href="${userUrl}">${fromUser.name}</a> ${action}` : action;
  ...
  let html = `
    ...
    `;
  html = await inlineCss(notificationStyle + html, {
