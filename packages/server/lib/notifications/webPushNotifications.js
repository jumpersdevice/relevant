...
function initWebPush() {
  try {
    webPush.setVapidDetails('https://relevant.community/', process.env.VAPID_PUBLIC_KEY, process.env.VAPID_PRIVATE_KEY);
...
exports.handleWebNotifications = async params => {
  try {
    ...
    if (!user.notificationSettings.desktop.all || !user.desktopSubscriptions || !user.desktopSubscriptions.length) {
      return;
    }
    ...
    const options = {};
    const processed = user.desktopSubscriptions.map(subscription => webPush.sendNotification(subscription, JSON.stringify(payload), options));
