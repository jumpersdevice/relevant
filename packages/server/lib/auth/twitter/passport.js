...
const auth = require("../auth.service"); // Handles both login and signup via http POST request - native
...
exports.nativeAuth = async (req, res, next) => {
  try {
    ...
    const user = await handleTwitterAuth({
      req,
      twitterAuth,
      profile,
      invitecode
...
async function handleTwitterAuth({
  ...
  const connectedToUser = await isConnectedToDifferentUser({
    user,
    profile
  });
  ...
  if (isNewUser) {
    user = await addNewTwitterUser({
      handle,
      invitecode
    });
    user = await addTwitterProfile({
      profile,
      user,
      twitterAuth
    });
...
async function getProfile(props) {
  ...
  const profile = await userProfile(authToken, authTokenSecret, {
    url,
    user_id
  });
  return profile;
...
async function addTwitterProfile({
  profile,
  twitterAuth,
  user
}) {
  ...
  const twitterId = profile.id; // TODO include twitter bio URL?
...
async function addNewTwitterUser({
  handle
}) {
