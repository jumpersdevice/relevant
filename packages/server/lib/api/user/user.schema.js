...
var remove = Object.keys(_user.default.schema.paths).filter(key => _user.default.schema.paths[key].options.select === false);
var customizationOptions = {};
var TC = (0, _graphqlComposeMongoose.composeWithMongoose)(_user.default, customizationOptions); // async function testSub() {
...
remove.map(field => TC.extendField(field, {
  ...
  resolve: (source, args, context) => {
    var isOwner = context.user && source._id.equals(context.user._id);
...
var userMutation = {}; // schemaComposer.Mutation.addFields({
