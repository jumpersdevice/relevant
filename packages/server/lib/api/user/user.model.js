...
UserSchema.pre('save', async function preSave(next) {
  try {
    this.postCount = await this.model('Post').countDocuments({
      user: this._id
    });
...
UserSchema.pre('remove', async function preRemove(next) {
  try {
    await this.model('CommunityMember').deleteMany({
      user: this._id
    }).exec();
    next();
  } catch (err) {
...
UserSchema.methods.addReward = async function addReward({
  type,
  user,
  extraRewards
}) {
  const amount = (0, _globalConstants.getRewardForType)(type) + (extraRewards || 0);
  const airdropTokens = Math.min(amount, _globalConstants.MAX_AIRDROP - this.airdropTokens);
...
UserSchema.methods.initialCoins = async function initialCoins(invite) {
  const airdropTokens = (0, _globalConstants.newUserCoins)(this, invite);
...
UserSchema.methods.updatePower = function updatePower() {
  ...
  const voteRegen = Math.max(elapsedTime / _globalConstants.POWER_REGEN_INTERVAL * 1);
...
UserSchema.methods.ensureParam = async function ensureParam(param) {
  ...
  const getParam = await this.model('User').findOne({
    _id: this._id
  }, '+param');
