...
var memberQuery = {
  ...
  myMemberships: TC.getResolver('findMany').wrapResolve(next => rp => {
    ...
    rp.args.filter = _objectSpread(_objectSpread({}, rp.args.filter), {}, {
...
function onlyOwner(resolvers) {
  Object.keys(resolvers).forEach(k => {
    resolvers[k] = resolvers[k].wrapResolve(next => /*#__PURE__*/function () {
...
var memberSubscription = {
  updateUnread: {
    ...
    subscribe: (0, _graphqlSubscriptions.withFilter)(() => _pubsub.pubsub.asyncIterator(UPDATE_UNREAD), (payload, args) => args.communities.includes(payload.communityId))
  }
};
exports.memberSubscription = memberSubscription;

var memberMutation = _objectSpread({}, onlyOwner({
