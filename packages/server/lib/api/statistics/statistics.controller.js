...
exports.user = async (req, res, next) => {
  try {
    ...
    const stats = await _statistics.default.find({
      user,
      date: {
        $gte: start,
        $lt: end
...
exports.change = (req, res, next) => {
  ...
  return _statistics.default.find(query).exec((err, stats) => {
    ...
    const startObject = stats.find(s => // console.log(s.startTime == new Date(startTime))
    // console.log('STAT START ', s.startTime.getTime());
    // console.log(startTime);
    s.startTime.getTime() === startTime);
    let startAmount;
    if (!startObject) startAmount = 0;else startAmount = startObject.hours[startHour] || 0;
    const endAmount = stats[stats.length - 1].hours[endHour] || 0;
    let change;
    if (startAmount) change = (endAmount - startAmount) * 100 / startAmount;else change = 0;
    const response = {
      change,
      startAmount,
      endAmount
