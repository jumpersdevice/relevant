...
InvestSchema.methods.removeVote = async function removeVote({
  post,
  user
}) {
  const vote = this;
  ...
  const earning = await _earnings.default.findOne({
    user: user._id,
    post: post._id
  });
...
InvestSchema.methods.placeBet = async function placeBet({
  ...
  const shares = (0, _post.computeShares)({
    post,
    stakedTokens
  });
  const postData = await this.model('PostData').findOne({
    post: post._id,
    communityId
  ...
  const communityInstance = await this.model('Community').findOne({
    _id: communityId
  });
...
InvestSchema.statics.createVote = async function createVote({
  ...
  const isManualBet = user.notificationSettings.bet.manual && communityInstance.betEnabled;
  if (!post.data) throw new Error(`Missing post data ${post.id} ${community} ${communityId}`);
  ...
  try {
    const stakedTokens = Math.abs(amount) * _globalConstants.VOTE_COST_RATIO * (user.balance + user.tokenBalance);
...
async function updateUserEarnings({
  ...
  const earningExists = await _earnings.default.countDocuments(lookup);
  const earning = await _earnings.default.findOneAndUpdate(lookup, {
    shares: vote.shares,
