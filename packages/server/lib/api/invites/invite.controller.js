...
exports.index = async (req, res, next) => {
  ...
  try {
    ...
    invites = await _invite2.default.find(query).populate('registeredAs').sort({
      updatedAt: -1
...
exports.createInvites = async invites => {
  invites = invites.map(async invite => {
    const code = (0, _v.default)();
    invite = new _invite2.default({ ...invite,
      code
    });
...
exports.create = async (req, res, next) => {
  try {
    ...
    if (!user.relevance) {
      user.relevance = await _communityMember.default.findOne({
        user: user._id,
        communityId
      });
    }
    ...
    const unusedInvites = await computeInviteNumber({
      user,
      communityId
    });
...
async function computeInviteNumber({
  user,
  communityId
}) {
  if (!user.relevance) {
  ...
  const totalInvites = (0, _globalConstants.totalAllowedInvites)(user.relevance.pagerank);
  const usedInvites = await _invite2.default.countDocuments({
    invitedBy: user._id,
    communityId,
    type: 'referral'
  });
...
exports.sendEmailFunc = async function inviteEamil(_invite) {
  ...
  if (invite && !invite._id) {
    invite = await _invite2.default.findById(invite);
  }
  ...
  let html = `
      ...
      `;
  html = await inlineCss(emailStyle + html, {
...
exports.adminInvite = async (req, res, next) => {
  try {
    ...
    user = await exports.handleAdminInvite({
      user,
      invitecode
    });
...
exports.handleAdminInvite = async ({
  user,
  invitecode
}) => {
  if (!invitecode) throw new Error('No invitation code');
  const invite = await _invite2.default.findOne({
    code: invitecode,
    type: 'admin',
    redeemed: {
      $ne: true
  ...
  await invite.save();
  const relevance = await _communityMember.default.findOne({
    user: user._id,
    communityId
  });
...
exports.count = async (req, res, next) => {
  try {
    ...
    const number = await computeInviteNumber({
      user,
      communityId
    });
    return res.status(200).json({
...
exports.destroy = async (req, res) => {
  ...
  try {
    await _invite2.default.findById(inviteId).remove();
  } catch (err) {
    return handleError(res)(err);
  }
