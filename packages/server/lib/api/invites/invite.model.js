...
InviteSchema.statics.processInvite = async function processInvite({
  ...
  if (invite) return referralRewards({
    invite,
    user,
    Invite: this
  });
  const publicInvite = await this.model('User').findOne({
    handle: invitecode
  });
  if (publicInvite) return publicReward({
...
async function publicReward({
  user,
  inviter
}) {
  await inviter.addReward({
    type: 'publicLink',
    user
  });
...
async function referralRewards({
  ...
  await invite.save();
  let inviter = await Invite.model('User').findOne({
  ...
  inviter = await inviter.addReward({
    type: 'referral',
    user
  ...
  const {
    author: updatedUser
  } = await (0, _computeApproxPageRank.default)({
    communityId,
    author: user,
    vote,
    user: inviter
