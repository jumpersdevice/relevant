...
exports.create = (req, res, next) => {
  ...
  return newNotification.save().then(() => {
    const newNotifObj = {
...
exports.show = (req, res, next) => {
  ...
  Notification.find(query).limit(limit).skip(skip).sort({
    _id: -1
  }) // .populate('byUser')
  .populate({
    path: 'byUser',
  ...
  }).then(notifications => res.status(200).json(notifications)).catch(next);
};
...
exports.unread = (req, res, next) => {
  ...
  Notification.countDocuments(query).then(unread => {
    res.status(200).json({
      unread
    });
  }).catch(next);
...
exports.showGeneral = (req, res, next) => {
  ...
  Notification.find(query).limit(limit).skip(skip).sort({
    _id: -1
  }).populate('byUser post tag').then(notifications => res.status(200).json(notifications)).catch(next);
};
...
exports.markRead = (req, res, next) => {
  ...
  return Notification.updateMany(query, {
    ...
    multi: true
  }).then(() => res.status(200).send()).catch(next);
};
