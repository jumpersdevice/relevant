...
const socketEvent = require("../../socket/socketEvent").default;
...
PostSchema.methods.updateClient = function updateClient(user) {
  ...
  const post = this.toObject(); // Prevent over-writing post object
...
async function updateLatestComment({
  post,
  communityId
}) {
  if (post.parentPost) return post;
  const latestComment = await post.model('Post').findOne({
    parentPost: post._id,
    communityId,
    hidden: {
      $ne: true
...
PostSchema.methods.updateRank = async function updateRank({
  communityId,
  updateTime
}) {
  let post = this;
  ...
  try {
    if (!post.data) {
      post.data = await post.model('PostData').findOne({
        post: post._id,
        communityId
      });
    }
    ...
    if (updateTime && !post.parentPost) {
      post = await updateLatestComment({
        post,
        communityId
      });
    ...
    let rank = pagerank < 0 ? 0 : postDate.getTime() / TENTH_LIFE + Math.log10(pagerank + 1);
    rank = Math.round(rank * 1000) / 1000; // But if a comment ranks highly - update post rank

    const topComment = await post.model('PostData').findOne({
      parentPost: post._id,
      communityId
    }, 'rank pagerank').sort({
      rank: -1
...
PostSchema.statics.newLinkPost = async function newLinkPost({
  linkObject,
  postObject
}) {
  const {
  ...
  let post = await this.model('Post').findOne({
    url,
    type: 'link'
  }).populate({
    path: 'data',
  ...
  if (!hidden) await post.updateRank({
    communityId
  });
...
PostSchema.methods.upsertLinkParent = async function upsertLinkParent(linkObject) {
  const post = this;
  const parent = await this.model('Post').newLinkPost({
    postObject: post,
    linkObject
  });
  parent.commentCount++;
...
PostSchema.methods.insertIntoFeed = async function insertIntoFeed(communityId, community) {
  try {
    const post = this;
...
PostSchema.methods.upsertMetaPost = async function upsertMetaPost(metaId, linkObject) {
  try {
    ...
    if (metaId) {
      ...
      if (_id) meta = await MetaPost.findOne({
        _id
      });
    }

    const url = linkObject ? linkObject.url : this.url || this.link;
    if (url && !meta) meta = await MetaPost.findOne({
      url
    });
...
async function getPostData({
  ...
  if (!post.data) {
    post.data = await this.model('PostData').findOne({
      post: post._id,
      communityId
    });
  }
...
PostSchema.methods.addTags = async function addTags({
  tags,
  communityId
}) {
  try {
    ...
    post.tags = combinedTags;
    post.data.tags = combinedTags;
    await post.data.save(); // linkObject.tags = combinedTags;
...
PostSchema.statics.sendOutMentions = async function sendOutMentions(mentions, post, mUser, comment) {
  try {
    ...
    const promises = mentions.map(async mention => {
      ...
      const blocked = mUser.blockedBy.find(u => u === mention) || mUser.blocked.find(u => u === mention);
      ...
      const users = await this.model('User').find(query, 'deviceTokens email name notificationSettings');
...
PostSchema.methods.pruneFeed = async function pruneFeed({
  communityId
}) {
  const post = this;
  ...
  const children = await this.model('Post').countDocuments({
    parentPost: post._id
  ...
  await this.model('PostData').deleteOne({
    post: post._id,
    communityId
  }).exec();
  return post;
...
async function updateParentPostOnRemovingChild(post) {
  ...
  let parent = await post.model('Post').findOne({
    _id: post.linkParent
  }).populate({
    path: 'data',
    match: {
  ...
  if (parent) parent = await parent.pruneFeed({
    communityId
  });
  ...
  if (shouldUpdateRank || shouldUpdateTime) {
    parent = await parent.updateRank({
      communityId,
      shouldUpdateTime
    });
  }
...
PostSchema.post('remove', async function postRemove(post, next) {
  ...
  const note = this.model('Notification').deleteMany({
    post: post._id
  }).exec();
  const feed = this.model('Feed').deleteMany({
    post: post._id
  }).exec();
  const data = this.model('PostData').deleteMany({
    post: post._id
  }).exec();
  let metaPost; // if (post.type === 'link' && !post.parentParent) {
  //   metaPost = await this.model('MetaPost')
  //   .deleteMany({ post: post._id })
  //   .exec();
  // }

  let commentNote; // remove notifications
