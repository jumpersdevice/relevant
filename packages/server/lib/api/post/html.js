...
exports.generatePreview = async (body, uri, reqUrl, noReadability) => {
  ...
  const miltiElement = ['news_keywords', 'keywords', 'article:tag', 'author', 'article:author'];
  const meta = $('meta');
  const $title = $('title');
  ...
  const originalUrl = (0, _url.parse)(uri);
  const cannonicalUrl = (0, _url.parse)(url);
  const k1 = data.keywords ? data.keywords.split(',').map(k => k.trim()) : [];
  const k2 = data.news_keywords ? data.news_keywords.split(',').map(k => k.trim()) : [];
  const k3 = data['article:tag'] ? data['article:tag'].split(',').map(k => k.replace('--primarykeyword-', '').trim()).filter(k => !k.match('--')) : [];
  let keywords = [...new Set([...k1, ...k2, ...k3])]; // general fix for mismatch
  ...
  const metaAuthor = data.author.split(/,|\sand\s/).map(a => a.trim()).filter(a => a !== '');
  ampAuthor = ampAuthor || [];
  ...
  let doc = _jsdom.default.jsdom(body, {
    features: {
      FetchExternalResources: false,
      ProcessExternalResources: false
    }
