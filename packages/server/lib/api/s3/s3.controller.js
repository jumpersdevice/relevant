...
exports.sign = (req, res) => {
  ...
  const base64Policy = Buffer.from(stringPolicy, 'utf-8').toString('base64'); // sign the base64 encoded policy

  const signature = crypto.createHmac('sha1', AWS_SECRET_KEY).update(Buffer.from(base64Policy, 'utf-8')).digest('base64'); // build the results object
