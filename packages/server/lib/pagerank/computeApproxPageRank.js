...
async function computeApproxPageRank({
  author,
  post,
  user,
  communityId,
  ...
  try {
    const com = await _community.default.findOne({
      _id: communityId
    ...
    if (author && !author.relevance) {
      author.relevance = await _communityMember.default.findOne({
        user: author._id,
        communityId
      });
    ...
    if (undoInvest) {
      postVotes = await _invest.default.countDocuments({
        post: post._id,
        ownPost: false
      });
      ...
      userVotes = await _invest.default.countDocuments({
        author: authorId,
        ownPost: false
      });
    ...
    const negPosRatio = member.pagerankRaw ? member.pagerankRawNeg / member.pagerankRaw : 0;
    ...
    if (post) {
      ...
      const normRank = Math.min(100, 100 * Math.log(N * pRank + 1) / Math.log(N * maxPostRank + 1));
      const normRankNeg = Math.min(100, 100 * Math.log(N * pRankNeg + 1) / Math.log(N * maxPostRank + 1));
    ...
    await Promise.all([post ? post.data.save() : null, author ? author.relevance.save() : null]);
    return {
      author,
      post
    };
