...
const defaultKeys = ['_id', 'id', 'createdAt', 'updatedAt', 'payoutTime', 'postDate', 'latestComment', 'rank'];

function sanitize(data, keys = defaultKeys) {
  if (!data) return data;
  ...
  const reducer = (result, key) => {
    ...
    if ((0, _lodash.isArray)(val)) return (0, _fp.set)(key, val.map(r => sanitize(r, keys)), result);
    if ((0, _lodash.isPlainObject)(val)) return (0, _fp.set)(key, sanitize(val, keys), result);
