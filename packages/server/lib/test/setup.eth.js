...
var provider = new _ethers.ethers.providers.Web3Provider(_ganacheCli.default.provider({
  network_id: 99,
  accounts: [{
    secretKey: '0x' + key,
    balance: 99e18
...
function _deployContract() {
  _deployContract = (0, _asyncToGenerator2.default)(function* () {
    try {
      var factory = new _ethers.ethers.ContractFactory(_RelevantToken.default.abi, _RelevantToken.default.bytecode, wallet);
...
function _initContract() {
  _initContract = (0, _asyncToGenerator2.default)(function* (contract) {
    ...
    var airdropRoundDecayBNString = new _bignumber.default(airdropRoundDecay.toString()).toFixed(0).toString();
    var firstNewAirdropBNString = new _bignumber.default(firstNewAirdrop.toString()).toFixed(0).toString();
    var initRoundRewardBNString = new _bignumber.default(initRoundReward.toString()).toFixed(0).toString();
    var timeConstantBNString = new _bignumber.default(timeConstant.toString()).toFixed(0).toString();
    var totalPremintBNString = new _bignumber.default(totalPremint.toString()).toFixed(0).toString();
    var roundDecayBNString = new _bignumber.default(roundDecay.toString()).toFixed(0).toString();
    yield contract.initialize(testName, testDecimals, testSymbol, testVersion, testDevFundAddress, initRoundRewardBNString, timeConstantBNString, targetInflation, targetRound, roundLength, roundDecayBNString, totalPremintBNString);
    yield contract.initializeRewardSplit(airdropSwitchRound, airdropRoundDecayBNString, firstNewAirdropBNString); // feature was added in contract upgrade
