...
var { validateTokenLenient, verify } = require('./auth/auth.service');

var whitelist = ['http://localhost:3000', 'https://relevant.community']; //white list consumers
...
var { db } = require('./config/db.connect'); // Persist sessions with MongoStore
...
server = app.listen(
  ...
  error => {
    if (error) {
      ...
      console.info(
        '==> \uD83C\uDF0E  Listening on port '
          .concat(port, '. Open up http://localhost:')
          .concat(port, '/ in your browser.')
      );
      var now = new Date();
