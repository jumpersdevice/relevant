...
class Graph {
  constructor(_ref) {
    var { votes: _votes, admins, community, usersWithDefault } = _ref;
    ...
    (0, _defineProperty2.default)(this, 'processVote', vote => {
      ...
      if (!user) return null;
      var { author, amount } = vote; // don't process own votes

      if (author && user._id.equals(author._id)) return null;
      var { post } = vote; // filter out older posts

      if (!IS_TEST) {
        var timeLimitPost = new Date(
          this.now.getTime() - _globalConstants.REP_CUTOFF_POSTS
        );
      ...
      var postWeight =
        Math.abs(a) * (1 / 2) ** (t / _globalConstants.RELEVANCE_DECAY_POSTS);

      var downvote = a < 0;
      var userNode = this.initUserNode(user);
      author &&
        this.handleAuthorNode({
          author,
          user,
          downvote,
          userWeight,
    ...
    (0, _defineProperty2.default)(this, 'processNegatives', () => {
      ...
      Object.values(this.userNodes).forEach(node => {
        ...
        this.userNodes.negConsumer.inputs = _objectSpread(
          _objectSpread({}, this.userNodes.negConsumer.inputs),
          {},
          {
            [node._id]: negDegree
          }
    ...
    (0, _defineProperty2.default)(this, 'cancelOutOpposites', _ref2 => {
      var { author, userNode, userWeight, downvote } = _ref2;
    ...
    (0, _defineProperty2.default)(this, 'handleAuthorNode', _ref3 => {
      var { author, downvote, userWeight, userNode } = _ref3;
      ...
      var authorNode = this.initUserNode(
        _objectSpread(
          _objectSpread({}, author.toObject()),
          {},
          {
            _id
          }
    ...
    (0, _defineProperty2.default)(this, 'handlePostNode', _ref4 => {
      var { post, userNode, downvote, postWeight } = _ref4;

      var _id = downvote ? post._id + '__neg' : post._id;

      var postNode = this.initPostNode(
        _objectSpread(
          _objectSpread({}, post.toObject()),
          {},
          {
            _id
          }
      ...
      postNode.inputs[userNode._id] = postNode.inputs[userNode._id]
        ...
        : postWeight; // userNode.degree += postWeight;
    ...
    (0, _defineProperty2.default)(this, 'initUserNode', (user, downvote) => {
      ...
      var start = user.relevance
        ? Math.max(
            downvote ? user.relevance.pagerankRawNeg : user.relevance.pagerankRaw,
            0
          )
        : 0;
