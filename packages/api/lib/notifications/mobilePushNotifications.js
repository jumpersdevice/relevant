...
function _handleMobileNotifications() {
  _handleMobileNotifications = (0, _asyncToGenerator2.default)(function* (user, alert, payload) {
    try {
      if (!user) return;
      ...
      if (!user.notificationSettings.mobile.all || !user.deviceTokens || !user.deviceTokens.length) {
        return;
      }
      ...
      var data = {
        ...
        custom: _objectSpread(_objectSpread({}, notePayload), {}, {
          toUser: user._id
        }),
