...
var { NODE_ENV } = process.env; // General problem: a user with some reputation who hasn't upvoted anyone
...
var create = /*#__PURE__*/ (function() {
  var _ref = (0, _asyncToGenerator2.default)(function*(req, res, next) {
    try {
      var { community, communityId } = req.communityMember;
      var communityInstance = yield _community.default.findOne({
        _id: communityId
      });
      var userId = req.user._id;
      var postId = req.body.post;
      var { amount } = req.body; // amount should be on a scale of -1 to 1 to prevent abuse;

      amount = Math.max(-1, amount);
      amount = Math.min(1, amount);
      var { post, user, author } = yield queryDb({
        userId,
        postId,
        communityId
      });
      ...
      post.data.expectedPayout = (0, _rewards.computePostPayout)(
        post.data,
        communityInstance
      );
...
function _getExistingVote() {
  _getExistingVote = (0, _asyncToGenerator2.default)(function*(_ref2) {
    var { user, post, amount, communityId } = _ref2;
...
function _updateSubscriptions() {
  _updateSubscriptions = (0, _asyncToGenerator2.default)(function*(_ref3) {
    var { post, user, amount, undoInvest, communityId, community } = _ref3;
...
function _sendAuthorNotification() {
  _sendAuthorNotification = (0, _asyncToGenerator2.default)(function*(_ref4) {
    var { author, user, post, type, undoInvest, amount } = _ref4;
    if (!author) return null; // Remove notification if undo;

    if (undoInvest) {
      yield _notification.default
        .deleteOne({
          type,
          post: post._id,
          forUser: author._id,
    ...
    var action =
      type === 'upvote'
        ? ' thinks your comment is relevant'
...
function _processNotifications() {
  _processNotifications = (0, _asyncToGenerator2.default)(function*(params) {
    var { post, user, amount, authorPagerank, undoInvest } = params;
    var { author } = params;

    if (!author) {
      if (amount < 0) return null;
      var children = yield _post.default
        .find({
          parentPost: post._id,
          parentComment: {
            $exists: false
    ...
    var pageRankChange = author.relevance
...
function _unhideTwitterComments() {
  _unhideTwitterComments = (0, _asyncToGenerator2.default)(function*(_ref6) {
    var { amount, post, communityId, community } = _ref6;
    if (!post) return null;
...
function _ensurePagerank() {
  _ensurePagerank = (0, _asyncToGenerator2.default)(function*(_ref7) {
    var { user, communityInstance } = _ref7;
    if (!user) return null;
...
function _queryDb() {
  _queryDb = (0, _asyncToGenerator2.default)(function*(_ref8) {
    var { userId, postId, communityId } = _ref8;
    ...
    var author = yield _user.default
      .findOne(
        {
          _id: post.user
        },
