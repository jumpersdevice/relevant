...
var getAssetsUrl = address => "https://map-api-direct.foam.space:443/user/".concat(address, "/assets");
...
function _checkCommunityAuth() {
  _checkCommunityAuth = (0, _asyncToGenerator2.default)(function* (_ref) {
    ...
    if (!ethLogin) {
      throw new Error('You need to connect the Ethereum address you use with FOAM in order to participate in this forum.\nYou can connect your address with Metamask via Wallet -> Connect Wallet.');
    }
    ...
    if (!totalPoints || totalPoints < points) {
      throw new Error("You can participate in this forum only after you have:\n - added ".concat(points, " points of interest to the FOAM map\n - have a balance of at least ").concat(tokens, " FOAM tokens"));
    }
    ...
    if (balance < tokens) throw new Error("You can participate in this forum only after you have:\n - added ".concat(points, " points of interest to the FOAM map\n - have a balance of at least ").concat(tokens, " FOAM tokens")); // if (!communityMember || !communityMember.role !== 'admin') {
