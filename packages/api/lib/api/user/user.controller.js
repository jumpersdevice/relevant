...
function _sendConfirmation() {
  _sendConfirmation = (0, _asyncToGenerator2.default)(function* (user, newUser) {
    ...
    var confirmUrl = "".concat(process.env.API_SERVER, "/user/confirm/").concat(user.handle, "/").concat(user.confirmCode);
    var data = {
      from: 'Relevant <info@relevant.community>',
      to: user.email,
      subject: 'Relevant Email Confirmation',
      html: "\n        Hi @".concat(user.handle).concat(text, "!\n      <br />\n      <br />\n        Please click on the link below to confirm your email address:\n      <br />\n      <br />\n      <a href=\"").concat(confirmUrl, "\" target=\"_blank\">Confirm Email</a>\n      <br />\n      <br />\n      ")
    };
...
exports.forgot = /*#__PURE__*/function () {
  var _ref = (0, _asyncToGenerator2.default)(function* (req, res, next) {
    ...
    try {
      ...
      var user = yield _user.default.findOne(query, 'resetPasswordToken resetPasswordExpires email handle');

      if (!user) {
        var errorText = email ? 'No user with this email exists' : "Couldn't find user with this username";
...
exports.testData = /*#__PURE__*/function () {
  var _ref7 = (0, _asyncToGenerator2.default)(function* (req, res, next) {
    try {
      ...
      var rel = yield _communityMember.default.find(query, 'pagerank level community communityId pagerankRaw').limit(limit).skip(skip) // .sort(sort)
      .populate({
        path: 'user',
...
exports.list = /*#__PURE__*/function () {
  var _ref8 = (0, _asyncToGenerator2.default)(function* (req, res, next) {
    try {
      ...
      var rel = yield _communityMember.default.find(query).limit(limit).skip(skip).sort(sort).populate({
        path: 'user',
...
exports.create = /*#__PURE__*/function () {
  var _ref9 = (0, _asyncToGenerator2.default)(function* (req, res, next) {
    try {
      ...
      user = yield user.save();
      if (invitecode) user = yield _invite.default.processInvite({
        invitecode,
        user
      });
...
exports.show = /*#__PURE__*/function () {
  var _show = (0, _asyncToGenerator2.default)(function* (req, res, next) {
    try {
      ...
      var relevance = yield _communityMember.default.find({
        user: user._id
      }).sort('-relevance').limit(5);
      var userObj = user.toObject();
      userObj.topTags = relevance || [];
      res.status(200).json(_objectSpread(_objectSpread({}, userObj), {}, {
        memberships
...
exports.block = /*#__PURE__*/function () {
  var _ref15 = (0, _asyncToGenerator2.default)(function* (req, res, next) {
    try {
      ...
      var sub1 = _subscription.default.deleteMany({
        following: user._id,
        follower: block
      }).exec();

      var sub2 = _subscription.default.deleteMany({
        following: block,
        follower: user._id
      }).exec();

      var feed1 = _feed.default.deleteMany({
        userId: user._id,
        from: block
      }).exec();

      var feed2 = _feed.default.deleteMany({
        userId: block,
        from: user._id
      }).exec();

      var results = yield Promise.all([userPromise, blockPromise, sub1, sub2, feed1, feed2]);
      user = results[0];
      return res.status(200).json(user);
    } catch (err) {
      return next(err);
...
exports.updateUserNotifications = /*#__PURE__*/function () {
  var _ref19 = (0, _asyncToGenerator2.default)(function* (req, res, next) {
    try {
      ...
      var newSettings = (0, _merge.default)(user.notificationSettings.toObject(), notificationSettings);
      ...
      if (subscription) {
        var findIndex = user.desktopSubscriptions.findIndex(s => s.endpoint === subscription.endpoint && s.keys && s.keys.auth === subscription.keys.auth && s.keys.p256dh === subscription.keys.p256dh);
...
exports.cashOut = /*#__PURE__*/function () {
  var _ref21 = (0, _asyncToGenerator2.default)(function* (req, res, next) {
    try {
      ...
      if (amount > maxClaim) throw new Error("You can not claim more than ".concat(maxClaim, " coins at this time."));
      ...
      var {
        sig,
        amount: bnAmount
      } = yield ethUtils.sign(address, amount);
      user.nonce = nonce;
      user.cashOut = {
        sig,
        amount: bnAmount,
